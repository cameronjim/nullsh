...
- **Allocator caveat during Phase 0 and 1**: modules use `nsh_malloc` from day one, but until Phase 2 those are thin wrappers over libc malloc. Phase 2 replaces the internals; call sites never change.
...
## Phase 5: `inspect`, the ELF viewer (claude-docs/plans/05-inspect-elf.md)
...
- The file header at offset 0: magic `\x7fELF`, class (32/64-bit), endianness, type (executable vs shared object vs relocatable, and note that modern `/usr/bin/ls` is ET_DYN because of PIE), entry point.
...
## Phase 7: `netmon`, the packet viewer (claude-docs/plans/07-netmon.md)
...
- `socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL))` delivers whole frames from one interface; requires CAP_NET_RAW (hence sudo, or `setcap` on the binary, and the plan doc covers both, plus the security tradeoff).
